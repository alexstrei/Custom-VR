...
static inline struct custom_vr *
custom_vr(struct xrt_device *xdev)
{
	return (struct custom_vr *)xdev;
}

DEBUG_GET_ONCE_LOG_OPTION(custom_vr_log, "CUSTOM_VR_LOG", U_LOGGING_WARN)
...
static void
custom_vr_destroy(struct xrt_device *xdev)
{
	struct custom_vr *hmd = custom_vr(xdev);
...
static xrt_result_t
custom_vr_update_inputs(struct xrt_device *xdev)
{
	/*
	 * Empty for the custom_vr driver, if you need to you should
...
static xrt_result_t
custom_vr_get_tracked_pose(struct xrt_device *xdev,
...
static void
custom_vr_get_view_poses(struct xrt_device *xdev,
...
xrt_result_t
custom_vr_get_visibility_mask(struct xrt_device *xdev,
...
custom_vr_create(void)
{
	...
	struct custom_vr *hmd = U_DEVICE_ALLOCATE(struct custom_vr, flags, 1, 0);
	...
	hmd->base.update_inputs = custom_vr_update_inputs;
	hmd->base.get_tracked_pose = custom_vr_get_tracked_pose;
	hmd->base.get_view_poses = custom_vr_get_view_poses;
	hmd->base.get_visibility_mask = custom_vr_get_visibility_mask;
	hmd->base.destroy = custom_vr_destroy;

	// Distortion information, fills in xdev->compute_distortion().
	u_distortion_mesh_set_none(&hmd->base);

	// populate this with something more complex if required
	// hmd->base.compute_distortion = custom_vr_compute_distortion;
